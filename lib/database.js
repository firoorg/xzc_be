var mongoose = require('mongoose')
  ...
  , MarketHistory = require('../models/markethistory')
...
function update_address(hash, txid, amount, type, cb) {
  ...
  find_address(hash, function(address) {
    if (address) {
      ...
      } else if (hash == 'zerospend') {
          console.log(address.txs);
          var tx_array = address.txs;
          tx_array.push({addresses: txid, denomination: amount/100000000});
          Address.update({a_id:hash}, {
              txs: tx_array,
              sent: address.sent + amount,
              balance: 0
...
function get_chart_data(chart, cb) {
  switch(chart) {
    case 'zeromint':
      Tx.aggregate([
        {"$match": {
          "vout.addresses": "Zeromint"
        }},
        {"$unwind": "$vout"},
        {"$match": {
          "vout.addresses": "Zeromint"
      ...
      function(err, results) {
        var last = results[results.length-1]._id,
            data = [],
            blocks = []
        results.forEach(function(result) {
          data[result._id] = result.value
        })
        for (var block = 1; block <= last; block++) {
          blocks.push({
            label: block,
            value: (data[block]) ? data[block] / 100000000 : 0
          })
        }
    ...
    case 'price-usd':
      ...
      function(err, results) {
        results.forEach(function(result, idx) {
          results[idx]['value'] = parseFloat(result['value']);
        });
...
module.exports = {
  ...
  // gets chart data for given chart
  get_chart: function(chart, cb) {
    get_chart_data(chart, function (err, data) {
      if(data) {
        return cb(data);
      } else {
        return cb(null);
  ...
  update_market_history: function(date, data, cb) {
    var ts = new Date(data.timestamp * 1000),
        fields = ['marketCap', 'price']
    fields.forEach(function(field) {
      for(var currency in data[field]) {
        data[field][currency] = parseFloat(data[field][currency])
      }
    })
    MarketHistory.update({
      ...
      cap: data.marketCap,
      price: data.price,
      volume:  parseFloat(data.volume24.btc),
      supply: data.availableSupplyNumber,
