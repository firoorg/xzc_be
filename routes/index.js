...
router.get('/charts/:chart', function(req, res) {
  var chart = req.params['chart'];
  if (settings.charts.enabled.indexOf(chart) != -1) {
    db.get_chart(chart, function(data) {
      if(data) {
        var chartdata = data.map(function(day) {
          return [day.label, day.value];
        })
...
router.get('/zerocoin', function(req, res) {
    res.render('zerocoin', { active: 'zerocoin', address: settings.address, hashes: settings.api });
});
